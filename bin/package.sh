...
get_package_variables_from_pom() {
  ...
  PACKAGE_VERSION=`grep "<version>" "${_ROOTT_DIR}/pom.xml" | head -n1 | tail -n1 | awk -F '[<>]' '{print $3}'`
  ...
  SQLSMITH_VERSION=`grep "<sqlsmith.version>" "${_ROOTT_DIR}/pom.xml" | head -n1 | awk -F '[<>]' '{print $3}'`
  PACKAGE_JAR_NAME="${PACKAGE_NAME}_${PACKAGE_VERSION}-with-dependencies.jar"
}
